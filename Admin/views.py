...
def district(request):
    if 'aid' in request.session:
        ...
        if request.method == "POST":
            ...
            tbl_district.objects.create(district_name=request.POST.get("txt_district"),state=state)
...
def viewcomplaints(request):
    if 'aid' in request.session:
        ...
        shopcom=complaint.objects.filter(shop__in=shop)
        coach=tbl_coach.objects.all()
        coachcom=complaint.objects.filter(shop__in=shop)
        return render(request,"Admin/ViewComplaint.html",{'user':usercom,'shop':shopcom,'caoch':coachcom})
    else:
        return redirect("webguest:Login")
...
def Reply(request,id):
    ...
    if request.method=="POST":
        Complaint.comaplaint_replay=request.POST.get('txt_reply')
        Complaint.comaplaint_status=1
        Complaint.save()
        
        return render(request,"Admin/Reply.html",{'msg':"Reply send succesfully"})
    else:
        return render(request,"Admin/Reply.html")


def ViewEnquiry(request):
    queries=tbl_queries.objects.all()
    return render(request,"Admin/ViewEnquiry.html",{"data":queries})


def ENReply(request,id):
    if request.method == "POST":
        en=tbl_queries.objects.get(id=id)
        name = en.queries_name
        email = en.queries_email
        question = en.queries_message
        enreply=request.POST.get("txt_reply")

        send_mail(
                    'Respected ' + str(name),#subject
                    "\rReply for "+str(question)+" is "
                    "\r\n"+ str(enreply),#body
                    settings.EMAIL_HOST_USER,
                    [email],
                )
        return redirect('xadmin:ViewEnquiry')
    else:
        return render(request,"Admin/ENReply.html")

...
def Complaint(request):
    if request.method=="POST":
        Complaint= complaint.objects.create(shop=tbl_shop.objects.get(id=request.session["sid"]),complaint_title=request.POST.get("txt_title"),complaint_content=request.POST.get("txt_content"))
...
def viewcomplaints(request):
    if 'sid' in request.session:
        ...
        return render(request,"Shop/ViewComplaint.html",{'shop':shopcom})
    else:
...
def Stock(request,id):
    if request.method=="POST":
        ...
        update=int(stock)+int(newstock)
        product.product_stock=update
        product.save()
        return redirect("webshop:Product")
    else:

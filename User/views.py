...
def SearchProduct(request):
    ...
    for i in product:
        wdata=tbl_product.objects.get(id=i.id)
        tot=0
        ratecount=tbl_rating.objects.filter(product=wdata).count()
        if ratecount>0:
            ratedata=tbl_rating.objects.filter(product=wdata)
            for j in ratedata:
    ...
    return render(request,"User/searchproduct.html",{"data":datas,"ar":ar})
...
def ajaxsearch(request):
    ...
    return render(request,"User/AjaxSearch.html",{"products":datas,"ar":ar})
...
def Mycart(request):
   if request.method=="POST":
     ...
     bookingdata.booking_amount=int(float(request.POST.get("carttotalamt")))
     ...
     for i in cartdata:
         i.cart_status='1'
         i.save()
     return redirect("webuser:checkout")
   else:
...
def ViewCoach(request,id):
    ...
    subcount =  tbl_subscribe.objects.filter(user=request.session["uid"],status=1,subscription__coach=coach).count()
    print(subcount)
    value = 0
    if subcount > 0:
        value = 1
...
def checkout(request):
    ...
    if request.method == "POST":
        bk.booking_amount=request.POST.get("txt_amount")
        bk.save()
        return redirect("webuser:pay")
    else:
        return render(request,"User/checkout.html",{"data":bk})



def Complaint(request):
    if request.method=="POST":
        Complaint= complaint.objects.create(user=tbl_user.objects.get(id=request.session["uid"]),complaint_title=request.POST.get("txt_title"),complaint_content=request.POST.get("txt_content"))
        return render(request,"User/Complaint.html")
    else:
        return render(request,"User/Complaint.html")
    
def pdtrating(request,mid):
    parray=[1,2,3,4,5]
    mid=mid
    wdata=tbl_cart.objects.get(id=mid)
    
    counts=0
    counts=stardata=tbl_rating.objects.filter(product=wdata.product_id).count()
    if counts>0:
        res=0
        stardata=tbl_rating.objects.filter(product=wdata.product_id).order_by('-datetime')
        for i in stardata:
            res=res+i.rating_data
        avg=res//counts
        return render(request,"User/pdtRating.html",{'mid':mid,'data':stardata,'ar':parray,'avg':avg,'count':counts})
    else:
         return render(request,"User/pdtRating.html",{'cid':mid})

def pdtajaxrating(request):
    parray=[1,2,3,4,5]
    rating_data=request.GET.get('rating_data')
    user_name=request.GET.get('user_name')
    user_review=request.GET.get('user_review')
    ...
    tbl_rating.objects.create(user_name=user_name,user_review=user_review,rating_data=rating_data,product=tbl_product.objects.get(id=wdata.product_id))
    stardata=tbl_rating.objects.filter(product=wdata.product_id).order_by('-datetime')
    return render(request,"User/AjaxRating.html",{'data':stardata,'ar':parray})

def pdtstarrating(request):
    r_len = 0
    five = four = three = two = one = 0
    car_id = request.GET.get("pdt")
    cdata = tbl_cart.objects.get(id=car_id)
    rate = tbl_rating.objects.filter(product=cdata.product_id)
...
def Following(request):
    following= tbl_follow.objects.filter(user=request.session["uid"])
    return render(request,"User/Following.html",{"data":following})



def AddWishList(request,id):
    tbl_wishlist.objects.create(user=tbl_user.objects.get(id=request.session["uid"]),product=tbl_product.objects.get(id=id))
    return redirect("webuser:ProductDetails",id)

def wishlist(request):
    wl = tbl_wishlist.objects.filter(user=request.session["uid"])
    return render(request,"User/WishList.html",{"data":wl})

def remove_list(request,id):
    tbl_wishlist.objects.get(id=id).delete()
    return redirect("webuser:wishlist")

def viewcomplaints(request):
    if 'uid' in request.session:
        user=tbl_user.objects.all()
        usercom=complaint.objects.filter(user__in=user)
        return render(request,"Coach/ViewComplaint.html",{'user':usercom})
    else:
        return redirect("webguest:Login")